...
(function ($) {
    ...
    var rangeSlider = $(".price-range"),
        ...
        minValue =
            rangeSlider.data("min-value") !== undefined &&
            rangeSlider.data("min-value") !== ""
                ? rangeSlider.data("min-value")
                : minPrice,
        maxValue =
            rangeSlider.data("max-value") !== undefined &&
    ...
    rangeSlider.slider({
        ...
        slide: function (event, ui) {
            // Hiển thị giá trị trong các ô input
            minamount.val("$" + ui.values[0]);
            maxamount.val("$" + ui.values[1]);
        },
    });

    // Đặt giá trị mặc định cho các ô input
    minamount.val("$" + rangeSlider.slider("values", 0));
    maxamount.val("$" + rangeSlider.slider("values", 1));

    // Ghi log khi giá trị thay đổi
    rangeSlider.on("slidechange", function (event, ui) {
        console.log("Final Values:", ui.values);
    });
...
function removeCart(rowId) {
    $.ajax({
        ...
        success: function (response) {
            ...
            //Xu ly o trong trang cart/index
            ...
            if (qty === 0) {
                ...
                cart_existItem
                    ...
                    .text(
                        "$" +
                            (
                                response["cart"].price * response["cart"].qty
                            ).toFixed(2)
                    );
            }
